...
export default function NichePage({ onBack, onContinue }: NichePageProps) {
  ...
  useEffect(() => {
    const checkForCompanyData = () => {
      ...
      console.log('Checking for company data (niche):', companyData);
      
      if (companyData?.data) {
        console.log('Company data found (niche):', companyData.data);
        setPrefillValue(companyData.data.niche || '');
        setIsWaitingForData(false);
      } else {
  ...
  const handleContinue = async (niche: string) => {
    // Save user response with timestamp
    if (niche !== prefillValue || !getUserData('niche')) {
      saveUserData('niche', {
        answer: niche,
  ...
  return (
    <QuestionPage
      title="Qual é o nicho do seu negócio?"
      subtitle="Descreva a sua indústria ou especialização"
      inputType="text"
      placeholder="ex: Desenvolvimento de soluções de IA"
